...
void HashTable::QuadraticAddUser(std::string userName, std::string password)
{
  bool mustRehash = false;
  //check if needs rehashed
  ...
  {
    if(FindNameMatchQuadratic(userName, password))
    {
      noDuplicate = false;
    }
  }
...
void HashTable::rehashQuadratic()
{
  ...
  for(int i=0;i<m_quadraticSize;i++)
  {
    temp[i].setEntry(m_quadraticArr[i].getuserName(), m_quadraticArr[i].getPassword());
  }
  ...
  for(int i=0;i<tempSize;i++)
  {
    if(!temp[i].isEmpty())
    {
      QuadraticAddUser(temp[i].getuserName(),temp[i].getPassword());
    }
  }
...
bool HashTable::FindNameMatchQuadratic(std::string userName, std::string password)
{
  for(int i=0;i<m_quadraticSize;i++)
  {
